        ...
        for line in temp:
            ...
            if line.startswith('PING'):
                ...
                if re.match('^!afk', message.lower()):
                	...
                	if user.lower() == CHANNEL:
                		tokens = message.split()
                		r = requests.put('https://api.twitch.tv/kraken/channels/xcouchleaguex', data=json.dumps({'channel': {'status': message[7:].strip(), 'delay': 0 }}), headers={'Accept': 'application/vnd.twitchtv.v3+json', 'Authorization': 'OAuth '+STREAM_PASS, 'Content-Type': 'application/json'})
                	...
                	if user.lower() == CHANNEL:
                		tokens = message.split()
                		r = requests.put('https://api.twitch.tv/kraken/channels/xcouchleaguex', data=json.dumps({'channel': {'game': message[6:].strip(), 'delay': 0 }}), headers={'Accept': 'application/vnd.twitchtv.v3+json', 'Authorization': 'OAuth '+STREAM_PASS, 'Content-Type': 'application/json'})
